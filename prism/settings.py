...
import sys, os, json, prism

# Confing utilities
def load_config():
	# Load config
	global PRISM_CONFIG
	PRISM_CONFIG = json.loads(open(os.path.join(CONFIG_FOLDER, 'config.json')).read())

def save_config():
	# Write to config
	with open(CONFIG_FILE, 'w') as file:
		json.dump(PRISM_CONFIG, file, indent=4, sort_keys=True)

def import_folders(path):
	# Snippet to import libraries.
	# 
	# This adds all folders within the "libs" folder.
...
def init(pid):
	global PANEL_PID, PRISM_PATH, PLUGIN_PATH, TMP_PATH, CONFIG_FOLDER, CONFIG_FILE, PRISM_CONFIG
	
	PANEL_PID = pid
	PRISM_PATH = os.path.dirname(os.path.realpath(__file__))
	
	PLUGIN_PATH = os.path.join(PRISM_PATH, 'plugins')
	...
	import_folders(os.path.join(PRISM_PATH, 'libs'))
